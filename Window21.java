...
for (int i = 0; i < 3; i++) {
graph.setColor(colors[2-i]);
graph.fillRect((1250/3)+i*200, 670, 10, 10);
graph.setColor(Color.black);
graph.drawString("Sucursal " + String.valueOf(i+1), (1350/3)+i*200,
680);
}
...
g2d.drawString(months[i], X+(i*100),620);
int maxHeight =
(int)(Math.round((values[i][0]+values[i][1]+values[i][2])*.5));
for (int j = 0; j < 3; j++) {
graph.setFont(new Font("Arial", 1, 16));
graph.setColor(colors[j]);
graph.fillRect(posX, 550-maxHeight, 40,
(int)(Math.round(values[i][j]*.5)));
graph.setColor(Color.black);

graph.drawString(String.valueOf((values[i][j])), posX+2,
(int)((550-maxHeight)+(Math.round(values[i][j]/4))));
maxHeight-=(int)(Math.round(values[i][j]*.5));
}
}
}
